---
title: 教学博客
---

## 教学博客

教学反思、AI 工具使用和课程设计。

## 文章

- [TMUA 题库分享包使用指南](./tmua_question_bank_share_pack_guide)
- [做 TMUA 题库，先管好数据，再做网页](./tmua_question_bank_workflow)
- [数学老师为什么要读一点数学史](./why_math_teachers_should_read_history)
- [指数不只是“乘几次”](./exponent_not_repeated_multiplication)
- [为什么矩阵乘法不是逐项相乘](./matrix_multiplication_not_entrywise)
- [auxiliary equation 到底省掉了什么](./auxiliary_equation_operator_factorization)
- [从三次方程到复指数](./complex_numbers_to_complex_exponential)
- [AI Agent 时代，老师如何搭建自己的个人教学网站](./ai_agent_teacher_website)
- [一节数学史课是怎么备出来的](./math-history-course)
- [从 Mathpix 到 Agent：一个数学老师的 AI 工具流水线](./tool-pipeline)
- [用 AI 写讲义之后，我发现自己差点不会教书了](./ai-handout-reflection)
