...
|---|-------|------------|----------------|
| 1 | Pure Mathematics | Challenging | Integration by parts, compound-angle formula, induction |
| 2 | Pure Mathematics | Challenging | Implicit differentiation, discriminants, hyperbolic functions, curve symmetry |
| 3 | Pure numbers | Hard | Complex exponential form, rotation formula, center of gravity formula, module length calculation |
| 4 | Pure numbers | Hard | Reflection matrix formula, matrix equation solution, rotation matrix, transformation compound |
...
=\left[\frac{\sin(2m+1)x}{2m+1}\right]_0^{\pi/2}
=\frac{(-1)^m}{2m+1}.
$$ 
...
Assume the formula holds for  $n=k$ , namely
 ...
 $$
I(k,k+2m+1)
=(-1)^m\frac{2^k k!(2m)!(k+m)!}{m!(2k+2m+1)!}.
$$ 
...
I(k+1,k+2m+2)
=(-1)^m
\frac{(k+1)2^k k!(2m)!(k+m)!}{m!(2k+2m+3)(2k+2m+1)!}.
$$ 
...
Since  $2k+2m+2=2(k+m+1)$ , this is
 ...
 $$
(-1)^m
\frac{2^{k+1}(k+1)!(2m)!(k+m+1)!}{m!(2k+2m+3)!},
$$ 
...
About 70% of the candidates attempted this question and the average score was about 63%. Common problems in Part (i) are algebraic and notational errors, especially the use of compound angle formulas. The formulation of induction in part (ii) is often unstandardized: base cases are omitted, induction assumptions are not made explicit, or it is not stated that the case of non-negative integers should start with $n=0$.
...
84% of candidates attempted this question and the average score was about 45%. Parts (i)(ii) are generally well done, but the strict lower bound in part (ii) is often ignored. When drawing part (iii), many answers did not make full use of the boundary conditions in part (ii), and the inflection points and intercepts were incompletely marked. Symmetry of the curve about $y=x$ and associated straight line boundaries are key to plotting.
...
This is a purely numerical question that few people choose. Common mistakes in part (i) are reversing the direction of rotation or missing the translation term. In part (ii), many candidates try to prove in both directions; the proof starting from $Q_2$ is usually clearer. Part (iii) requires the calculation of modular lengths using complex numbers, and it is a common mistake to mistake the relevant complex numbers for real numbers.
...
This question is not very popular, but the basic ideas in each quiz are usually correct. In part (i), most candidates can use symmetry to find the images of $\mathbf{j}$ and $\mathbf{k}$ even without fully using the question stem. Part (ii) of the equation is more straightforward to solve, but calculation errors are more common. Part (iii) requires techniques analogous to part (i). A common mistake when describing the transformation in part (iv) is to say "rotation about the origin" without clarifying that this is a half rotation.
...
This question is more popular and has a higher success rate. The main problem in part (i)(a) is that there is an error when evaluating $A$, or the undefined at $x=k$ is ignored. Many candidates in Part (i)(b) do not realize that $x$ needs to be differentiated on both sides. Derivative errors and variable confusion are the main obstacles. Part (ii) Most candidates can use the results of Part (i)(b), but the treatment of the logarithmic term in the limit is often missed.
...
About 70% of candidates attempted this question, but the average score was only about 35%. Most candidates can complete Part (i), but the endpoint slope is often ignored. The drawing quality of part (ii) is weak and the endpoint behavior is not handled well. The small approximation arguments in part (iii) are often incomplete; many candidates fail to realize that $C_1$ is a branch of $C_2$. Polar area calculations are a major hurdle, and successful people usually realize that they need to take the difference between two polar integrals.
...
This is the second most popular and most successful question, with an average score of about 65%. Part (i) can be completed by almost all candidates. In Part (ii), finding a first-order differential equation is usually not the main problem; many candidates will first guess $n=-1$ and then verify $k=2$. Integration factors and integral calculations are the main points lost. Some candidates use the method of adding special solutions plus homogeneous solutions, but homogeneous solutions often make mistakes.
...
About 60% of candidates attempted this question, but the average score was only about 33%. A common mistake in part (i) is to mistakenly assume that all even-numbered subscript terms satisfy the required properties. Part (ii) requires considering the two adjacent cases $(u_{2k-1},u_{2k})$ and $(u_{2k},u_{2k+1})$ respectively. The understanding of "consecutive" in part (iii) is key, as many candidates mistakenly claim that the recurrence must occur in a location such as $2^n k$. Parts (iv)(v) are rarely completed and require connecting the Euclidean algorithm idea of ​​part (iv) with the surjectivity of part (v).
