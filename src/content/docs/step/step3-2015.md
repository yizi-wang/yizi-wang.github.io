...
|---|-------|------------|----------------|
| 1 | Integral Calculus | Challenging | Integral by parts, integral by substitution $u=x-x^{-1}$, derivation of recursive formulas, symmetry |
| 2 | Sequences and Inequalities | Challenging | Counterexample construction, mathematical induction, scaling of inequalities, transitivity of order relations |
| 3 | Polar Coordinates | Hard | Analysis of polar coordinate equations, discussion of different situations, integration of trigonometric functions, geometric properties of clam lines |
| 4 | Complex Numbers | Challenging | Intermediate value theorem, Vedic theorem, De Moivre's theorem, analysis of real and imaginary parts of complex numbers |
| 5 | Number Theory (Number Theory) | Challenging | Proof by contradiction, minimum element principle, proof of equivalent propositions, set theory methods |
| 6 | Pure Mathematics | Challenging | Solving simultaneous equations, discriminant analysis, counterexample construction, real number and complex number conditions |
| 7 | Pure Mathematics | Challenging | Mathematical induction, differential operator operations, binomial expansion, x=1 substitution method |
| 8 | Pure Mathematics | Hard | Polar coordinate transformation, chain rule, separation of variables, partial fraction integration, solution curve classification discussion |

---

## Question 1

**Topic**: Integral Calculus &nbsp;|&nbsp; **Difficulty**: Challenging &nbsp;|&nbsp; **Marks**: 20
...
**Topic**: Sequences and Inequalities &nbsp;|&nbsp; **Difficulty**: Challenging &nbsp;|&nbsp; **Marks**: 20
...
Counterexample: Set  $s_n = 1$  and  $t_n = 2$  when  $n$  is odd, and  $s_n = 2$  and  $t_n = 1$  when  $n$  is even. Both sequences consist of positive numbers.
...
**Topic**: Polar Coordinates &nbsp;|&nbsp; **Difficulty**: Hard &nbsp;|&nbsp; **Marks**: 20
...
-  $z_1 + z_2 + z_3 = S_1 = -a$ .
-  $z_1z_2 + z_1z_3 + z_2z_3 = \frac{S_1^2 - S_2}{2} = b$  (since  $S_1^2 = S_2 + 2(z_1z_2 + z_2z_3 + z_1z_3)$ ).
-  $z_1z_2z_3 = -c$  (from  $6c = -S_1^3 + 3S_1S_2 - 2S_3 = -6z_1z_2z_3$ , verified by expanding  $S_1^3$  and  $S_1S_2$ ).
...
**Topic**: Number Theory &nbsp;|&nbsp; **Difficulty**: Challenging &nbsp;|&nbsp; **Marks**: 20
...
**Topic**: Pure Mathematics &nbsp;|&nbsp; **Difficulty**: Challenging &nbsp;|&nbsp; **Marks**: 20
...
Treating the equations for  $u$  and  $v$  as simultaneous equations for  $w$  and  $z$ , one finds that  $w = \frac{u \pm \sqrt{2v - u^2}}{2}$  and  $z = \frac{u \mp \sqrt{2v - u^2}}{2}$  which demonstrates that if  $u$  and  $v$  are real and  $u^2 \le 2v$ , then  $w$  and  $z$  are real. If  $w$  and  $z$  are real, then  $u$  and  $v$  are (trivially) and  $2v - u^2 = (w - z)^2 \ge 0$ .
...
-  $w + z = 1 = u$ .
-  $w^2 + z^2 = (w + z)^2 - 2wz = 1 - \frac{2}{3} = \frac{1}{3} = u^2 - \frac{2}{3}$ .
-  $w^3 + z^3 = (w + z)^3 - 3wz(w + z) = 1 - 3 \cdot \frac{1}{3} \cdot 1 = 0$ , and  $\lambda u - \lambda = 1 \cdot 1 - 1 = 0$ .
...
About 3/5 candidates attempt this question but the success rate is not high. Part (i) requires proving bidirectional implication (if and only if), which stumps many candidates; (ii) the first question has a better chance of scoring, but attention should be paid to the fact that three different u values ​​cannot be generated when lambda=2; the counterexample part needs to clearly show that it is indeed a counterexample.
...
We prove the slightly stronger statement that if  $m$  is a positive integer and  $0 \leq n < m$ , then  $D^n(1-x)^m$  is divisible by  $(1-x)^{m-n}$ .

**Base case** ( $n = 0$ ):  $D^0(1-x)^m = (1-x)^m$ , which is divisible by  $(1-x)^{m-0} = (1-x)^m$ .

**Inductive step**: Suppose  $D^k(1-x)^m = g(x)(1-x)^{m-k}$  for some polynomial  $g(x)$  and some non-negative integer  $k < m - 1$ . Then
...
The bracketed expression is a polynomial in  $x$ , and the power of  $(1-x)$  is  $m - k - 1 = m - (k+1)$ . Since  $k + 1 < m$ , this is non-negative, so  $D^{k+1}(1-x)^m$  is divisible by  $(1-x)^{m-(k+1)}$ .

By induction,  $D^n(1-x)^m$  is divisible by  $(1-x)^{m-n}$  for all integers  $n$  with  $0 \leq n < m$ , hence in particular for all positive integers  $n < m$ .  $\blacksquare$ 
...
Tied with Q2 for third most popular and second most successful. There are many flaws in part (i), and many candidates are unable to make correct formal induction; in part (ii), some candidates believe that x and d/dx are interchangeable, and often infer the wrong D^n(1-x)^m formula from the first few terms. Not surprisingly, many people arrive at the correct answer by taking incorrect steps when deriving toward a given result (such as substituting x=1 into (1-x)^m before using the differential operator).
...
**Topic**: Pure Mathematics &nbsp;|&nbsp; **Difficulty**: Hard &nbsp;|&nbsp; **Marks**: 20
...
Cross-multiplying (assuming the denominator  $\frac{dr}{d\theta}\cos\theta - r\sin\theta \neq 0$ ):
 ...
 $$(\sin\theta + \cos\theta)\left(\frac{dr}{d\theta}\sin\theta + r\cos\theta\right) = (\sin\theta - \cos\theta)\left(\frac{dr}{d\theta}\cos\theta - r\sin\theta\right).$$ 
...
Expanding the cross-multiplied equation directly gives:
...
The  $\frac{dr}{d\theta}$  terms from  $LHS$  are:  $\sin^2\theta + \sin\theta\cos\theta - r^2\sin\theta\cos\theta$ . From  $-RHS$ :  $-\sin\theta\cos\theta + \cos^2\theta + r^2\sin\theta\cos\theta$ . So the coefficient of  $\frac{dr}{d\theta}$  is:
...
More than 80% of candidates attempted this question and it was the highest scoring question. Part (i) Many candidates did not realize that r is a function of theta; (ii) Partially transformed differential equations were generally correct, but the correct use of partial fraction integration after separation of variables was less frequent than expected. It's amazing how many people don't try to draw any diagram of the solution, even though the rest of it is done well or perfectly. No one realizes that the constant A in (ii) is truly arbitrary (since the logarithmic term in the integral has an absolute value). The drawing of solution curves tested the best students.
