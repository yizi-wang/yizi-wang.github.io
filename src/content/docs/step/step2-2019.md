...
|---|-------|------------|----------------|
| 1 | Algebra & Functions | Challenging | Product derivation rule, tangent equation establishment, algebraic simplification and factorization, proof of necessary and sufficient conditions |
| 2 | Calculus (Calculus) | Challenging | The geometric meaning of inverse functions, the principle of area addition, the integral substitution method, and the evaluation of implicit functions |
| 3 | Algebra & Functions | Challenging | Mathematical induction, triangle inequalities, polynomial valuation techniques, rational root theorem |
| 4 | Trigonometry | Hard | Double angle formula chain elimination method, logarithmic derivation method, limit calculation, infinite product and infinite series |
| 5 | Sequences & Series | Challenging | Fixed point equations, recursive sequence analysis, discriminant method, proof of necessary and sufficient conditions |
| 6 | Differential Equations | Challenging | Integral factor method, variable substitution, stationary point and extreme value analysis, qualitative differential equation theory |
| 7 | Geometry & Vectors | Challenging | Dot product operation, vector module length calculation, transformation of geometric conditions and vector conditions, symmetry analysis |
| 8 | Matrices & Linear Algebra (Matrices & Linear Algebra) | Hard | Properties of matrix multiplication, elementary matrix decomposition, functional equation reasoning, properties of determinants |
...
No official comment. Points that are prone to errors: (1) Omissions when deriving products and derivation; (2) The geometric meaning of "the tangent no longer intersects the curve" in part (ii) is unclear, and the tangent and curve equations need to be combined to analyze the multiple root conditions; (3) The question requires proof of "if and only if", so attention must be paid to bidirectional derivation.
...
No official comment. Points prone to errors: (1) The rigor of the area diagram method - it needs to be explained that monotonically increasing f ensures the existence of the inverse function; (2) The relationship between h(t) and g(t) in part (ii) is easy to ignore the translation transformation h(t) = g(t-2)+? In fact (h(t))³+h(t)=t+2 means some deformation of h(t)=g(t+2), which needs to be handled carefully; (3) The transformation of the integral limit must be accurate.
...
No official comment. Error-prone points: (1) The estimation of |f(x)-1| in part (i)(a) needs to be split item by item and then summed using a geometric series; (2) The demarcation treatment of |ω| The integer roots of need to be narrowed down by the rational root theorem and then verified one by one.
...
**Topic**: Trigonometry &nbsp;|&nbsp; **Difficulty**: Hard &nbsp;|&nbsp; **Marks**: 20
...
No official comment. Error-prone points: (1) The technique in part (i) - multiplying sin(π/9) to construct chain elimination - is not easy to think of; (2) In part (ii), the tan summation formula is obtained by taking the logarithm of the product and deriving the derivative. It is necessary to pay attention to the corresponding relationship between the product and the summation; (3) The condition of lim(sinθ/θ)=1 is required when taking the limit in part (iii), and the index transformation of the summation formula (j starts from 2) is prone to errors.
...
No official comment. Error-prone points: (1) The definitions of "constant sequence" and "period 2 sequence" need to be strictly distinguished - period 2 requires non-constants; (2) the analysis of the condition f(f(x))=x and f(x)≠x in part (i) of period 2 requires careful factorization; (3) the last question in part (ii) is a rhetorical question, the answer is "no", and a counterexample needs to be constructed.
...
No official comment. Error-prone points: (1) In part (i), the proof that there is no stationary point when k<-2 needs to be combined with the condition that the stationary point is on y=-x-1 and the straight line does not intersect with the solution curve; (2) in part (ii), dy/dx can be factorized into (x-y-1)(x-y-3), and identifying this structure is the key; (3) When drawing qualitatively, the straight-line solution and the stationary point trajectory need to be marked.
...
No official comment. Error-prone points: (1) In part (i), a·b=-1/2 requires further explanation |a-b|=√3, so the triangle is equilateral; (2) In part (ii)(a), the proof of the parallel tetrahedron requires the use of a₁·a₂=a₃·a₄ and the coplanarity condition; (3) In part (ii)(b), attention should be paid to the calculation of the side length of the regular tetrahedron √(8/3) a₁·a₂=-1/3.
...
No official comment. Error-prone points: (1) In part (ii), J²=I deduces f(J)²=1 and then f(J)≠1 to obtain f(J)=-1. This step of reasoning needs to be rigorous; (2) In part (iii), K=diag(1,k) is used to let k→0 to prove that f=0 when the rows are proportional. It is not easy to think of the technique; (3) In part (iv), P², P⁻¹ and K⁻¹PK properties to lock f(P)=1.
