...
|---|-------|------------|----------------|
| 1 | Functions and Curves | Standard | Derivatives and stationary points, Vieta formula, discriminant constraints, curve asymptote analysis, inequality proof |
| 2 | Sequences and Proof by Induction | Standard | Mathematical induction, product derivation rule, derivation of recursion relations, elimination method |
| 3 | Differential Equations | Challenging | Undetermined coefficient method, two integrals, discussion of separate cases, Euler-type equations |
| 4 | Hyperbola and Parametric Equations (Hyperbola and Parametric Equations) | Challenging | Parametric equations to find tangent lines, simultaneous equations to find intersection points, trigonometric identity simplification, vertical tangent line conditions |
| 5 | Mean Inequality (AM-GM Inequality) | Challenging | AM-GM Inequality, Auxiliary Function Method, Derivation to Determine Monotonicity, Mathematical Induction, Reversible Derivation |
| 6 | Complex Numbers | Hard | Complex conjugate operation, properties of complex numbers on the unit circle (aa*=1), complex number representation of collinear conditions, linear combination elimination |
| 7 | Trigonometry | Challenging | De Moivre's theorem, binomial theorem expansion, Vedic theorem (relationship between roots and coefficients), small angle approximation inequalities, limits |
| 8 | Integration (Integration) | Challenging | Variable substitution y=x^{-1}, partial fraction decomposition, summation of split terms (stretching series), properties of periodic functions, segmented processing of fractional partial functions |

---

## Question 1

**Topic**: Functions and Curves &nbsp;|&nbsp; **Difficulty**: Standard &nbsp;|&nbsp; **Marks**: 20
...
**Topic**: Sequences and Proof by Induction &nbsp;|&nbsp; **Difficulty**: Standard &nbsp;|&nbsp; **Marks**: 20
...
Differentiating and equating coefficients generates three simultaneous equations. Two of these can be written in terms of  $b+c-1$  and  $c$ , giving the two possible choices  $a = 1 + p \pm q$ ,  $b = 1 \mp 2q$ ,  $c = -p \pm q$ . Part (i) makes use of the form obtained in the stem which can be then integrated twice, with a minor algebraic rearrangement after each integration, with different cases arising as  $q = 0$  or not; the solutions are
...
Multiplying by  $x^a$ :
 ...
 $$x^a(x^b(x^c y)')' = x^{a+b+c}y'' + (b+2c)x^{a+b+c-1}y' + c(b+c-1)x^{a+b+c-2}y.$$ 

Comparing with  $x^2y'' + (1-2p)xy' + (p^2 - q^2)y$ :

 $$a + b + c = 2, \qquad b + 2c = 1 - 2p, \qquad c(b+c-1) = p^2 - q^2.$$ 

Put  $s = b+c-1$ . Then  $b+2c = s+c+1$ , so the second equation gives

 $$s+c = -2p, \qquad cs = p^2 - q^2.$$ 

Thus  $c$  and  $s$  are the roots of

 $$t^2 + 2pt + (p^2-q^2) = 0,$$ 

so  $t=-p\pm q$ . Taking  $c=-p+q$  and  $s=-p-q$  gives

 $$c=-p+q, \qquad b=s-c+1=1-2q, \qquad a=2-b-c=1+p+q.$$ 

The alternative choice  $c=-p-q$ ,  $b=1+2q$ ,  $a=1+p-q$  also works and yields the same solutions to the differential equation. Hence the required form exists.  $\qquad \square$ 

---

**Part (i)**

Using  $a=1+p+q$ ,  $b=1-2q$ ,  $c=-p+q$ , the equation  $x^a(x^b(x^c y)')' = 0$  becomes

 $$x^{1+p+q}\!\left(x^{1-2q}\!\left(x^{-p+q}y\right)'\right)' = 0.$$ 
...
With  $q = 0$ :  $a = 1+p$ ,  $b = 1$ ,  $c = -p$ . The equation becomes

 $$x^{1+p}\!\left(x\!\left(x^{-p}y\right)'\right)' = x^n.$$ 
...
**Case  $n = p$ :** The equation is  $x^2y'' + (1-2p)xy' + p^2y = x^p$ . Using the factored form and dividing by  $x^{1+p}$ :
 ...
 $$\left(x\!\left(x^{-p}y\right)'\right)' = x^{-1}.$$ 

First integration:  $x(x^{-p}y)' = \ln x + A$ .
...
**Topic**: Hyperbola and Parametric Equations &nbsp;|&nbsp; **Difficulty**: Challenging &nbsp;|&nbsp; **Marks**: 20
...
**Topic**: AM-GM Inequality &nbsp;|&nbsp; **Difficulty**: Challenging &nbsp;|&nbsp; **Marks**: 20
...
The first question is a simple basic conclusion, and there are many ways to prove it. Equality the expression with its conjugate (using the real number condition), applying the properties of the sum and product of the conjugate and sorting it out algebraically to obtain the second result of (i). Using the unit circle property to replace the conjugate of a and c, the final result of (i) is obtained through algebraic arrangement (including division by c-a which can be proved to be non-zero). Using Q on AC and combining the results of (i), similarly use Q on BD to obtain two equations, and linearly combine to obtain the equation about q* in (ii). Similar to the linear combination, the equation of q can also be obtained, and the second result of (ii) can be obtained by adding the existing results. (iii) The first result uses the conclusion of (i) that P is on AB and p is a real number. Similar results are obtained similarly for P on CD. Multiply the final result of (ii) by p, simplify using the two expressions just obtained, and divide by ac-bd to complete the proof. This question is essentially about poles and polar lines.
...
The least popular pure numerical questions were attempted by less than 40% and the average score was just over 5/20. There are many successful solutions to the first question, but a common mistake is treating q and c as scalar multiples of a, and the * operation confuses some candidates. Candidates entering (ii) can start well, but it is easier for candidates who use the unit circle condition to derive the final equation. Candidates who persisted to (iii) generally performed well, but occasionally had unaccounted for non-zero deductions when dividing by various factors.
...
Express cot θ as cos θ/sin θ and apply De Moivre's theorem to get the first result. Binomial expansion of the left-hand side of the equation, rearranged by odd-power terms and divided by 2i, simplifies to the left-hand side of the second result, which equals zero when (2n+1)θ = mπ (m=1,2,...,n). (ii) is obtained by considering the sum of the roots of the equation obtained in (i). The first result of (iii) starts from the given small angle inequality, paying attention to the sign of the reciprocal and the square, and then using the appropriate Pythagorean trigonometric identity. Sum the inequalities from m=1 to n, sort out the requirements and objects, and use the θ value defined in (i) to get the upper and lower bounds as n(2n-1)π²/(3(2n+1)²) and n(2n+2)π²/(3(2n+1)²) respectively. Both tend to π²/6 when n→∞.
...
The try rate was slightly higher than Q6, with an average of 8/20. There were no problems with the first part of (i), but fewer candidates went further; candidates who expanded (cot θ + i)^{2n+1} almost all succeeded, even if they could not always handle the condition of sin θ ≠ 0. Most candidates who attempted (ii) got full marks, but some forgot to divide C(2n+1,3) by C(2n+1,1). Almost all candidates who attempted (iii) proved the first result, but applying the results to draw the final conclusion was more difficult. A common mistake is to sum over θ instead of m, and the last part of the algebra is often wrong.
...
**Topic**: Points (Integration) &nbsp;|&nbsp; **Difficulty**: Challenging &nbsp;|&nbsp; **Marks**: 20
...
The first result of (i) can be proved by interpreting the sum of the integrals as a single integral from 1 to ∞ and substituting y=x^{-1}. The corollary can be made by splitting the integral into its partial fractions, substituting y=x+1 in the integral of the first fraction, using the periodicity condition, and then reducing the split-term sum to a single integral. The first integral of (ii) uses the result of (i) directly, observing that {x}=x within the integration range, and then integrating normally gives 1-ln 2. The second integral needs to observe {2(x+1)}={2x+2}={2x}. Use the result of (i) to split the integral into two integrals from 0 to 1/2 and from 1/2 to 1, respectively dealing with two different explicit expressions of {2x}; the result can be simplified to 2+ln(3/16) or an equivalent form.
...
The fifth most popular question, with an attempt rate of 5 out of 8, and an average score of just over 9/20, making it the fourth most successful question. The first result of (i) usually does well, but the second result is usually restricted to candidates who are found to be able to use partial fractions. The first integral of (ii) is usually completed correctly. But the second integral showed various errors, such as failing to see or explain how the first part was used and not simplifying the answer. The nature of periodicity and the need to integrate different expressions over different intervals are common stumbling blocks.
