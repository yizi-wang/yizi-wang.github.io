...
|---|-------|------------|----------------|
| 1 | Integration (Integration) | Standard | Trigonometric substitution $x+a=\sqrt{b-a^2}\tan u$, perfect square formula, derivation of recurrence relations, mathematical induction |
| 2 | Coordinate Geometry (Coordinate Geometry) | Challenging | Parametric representation $(at^2, 2at)$, implicit function derivation to find the normal equation, Vedic theorem ($q+r=-p, qr=2$), simultaneous equations to find the intersection point |
| 3 | Differential Equations | Challenging | Derivatives of expressions containing exponential functions, factor theorem/remainder theorem, comparison of polynomial degrees, proof by contradiction |
| 4 | Series and Sequences | Challenging | Split term method (telescopic summation), hyperbolic function definition $\text{sech}(ry) = \frac{2e^{-ry}}{1+e^{-2ry}}$, variable substitution $x=e^{-2y}$, using the properties of even functions to deal with negative subscripts |
| 5 | Combinatorics and Number Theory | Hard | Symmetry of binomial expansion, argument for divisibility of prime numbers, multiplication factorization $P_{1,2m+1} = P_{1,m+1} \cdot P_{m+1,2m+1}$, strong induction (discussed in odd and even categories) |
| 6 | Hyperbolic Functions (Hyperbolic Functions) | Challenging | Hyperbolic function addition formula, inverse hyperbolic function, classification discussion, necessary and sufficient condition demonstration |
| 7 | Complex Numbers | Hard | Unit root factorization, factor theorem, product of complex numbers modules, geometric properties of regular polygons |
| 8 | Functional Equations | Standard | Substituting -x to establish a system of equations, function iteration to construct loops, algebraic elimination method, verify solution |

---

## Question 1

**Topic**: Points (Integration) &nbsp;|&nbsp; **Difficulty**: Standard &nbsp;|&nbsp; **Marks**: 20
...
Let
 ...
 $$J = \int_{-\infty}^{\infty} \frac{t^2}{(t^2+c^2)^{n+1}}\,dt.$$ 

Since  $t^2=(t^2+c^2)-c^2$ ,

 $$J = I_n - c^2 I_{n+1}. \qquad \text{(*)}$$ 

Evaluate  $J$  by parts. Let  $u = t$  and  $dv = \frac{t}{(t^2 + c^2)^{n+1}} \, dt$ , so  $du = dt$  and  $v = \frac{-1}{2n(t^2 + c^2)^n}$ :
...
Substituting into  $(*)$ :
 ...
 $$I_n - c^2I_{n+1} = \frac{1}{2n}I_n.$$ 

Rearranging:

 $$c^2I_{n+1} = \frac{2n-1}{2n}I_n,$$ 

so

 $$2n c^2I_{n+1} = (2n-1)I_n.$$ 
...
This is the most commonly attempted question, answered by over 93% of candidates, with a high success rate (second highest), and an average score of about two thirds. Most candidates successfully completed (i) and (iii), but there were various algebraic errors, and some candidates forgot to substitute the integral limit in (i). Many candidates start after using the conversion of (i) in (ii), but get stuck when faced with the points earned.
...
**Topic**: Coordinate Geometry &nbsp;|&nbsp; **Difficulty**: Challenging &nbsp;|&nbsp; **Marks**: 20
...
About three-quarters of candidates attempted this question, with a moderate success rate. Most people use implicit function derivation rather than parametric methods, and can find equations for tangents, normals, and chords but not always factored simplifications. Those candidates who could factor made good progress, while those who could not factored struggled with (i). Other weaknesses include the failure to exploit the relationship between $r+q$ and $rq$, thereby not finding the fixed point in (ii). In the last part, it is not uncommon to consider only positive numbers after taking the square root of an inequality.
...
Differentiating, multiplying by denominators and dividing by the exponential function gives

 $$(x+1)^2\bigl(Q(P+P')-PQ'\bigr)=(x^3-2)Q^2,$$ 

which, on substituting  $x=-1$ , gives the first required result. Denoting the degree of  $P$  by  $p$  and that of  $Q$  by  $q$  in this expression yields  $p + q + 2 = 2q + 3$  and hence the desired result in (i). Furthermore, in the given case, substitution in the same result and postulating  $P(x) = ax^2 + bx + c$  yields consistent equations for  $a, b$  and  $c$  and thus  $P(x) = x^2 - 2x$ .

For part (ii), commencing as in part (i) demonstrates again that Q has a factor  $(x + 1)$ . Supposing  $Q(x) = (x + 1)^n S(x)$ , where  $n \ge 1$  and  $S(-1) \ne 0$ , with  $P(-1) \ne 0$  and substituting in the expression already derived leads to a contradiction by comparing the orders of vanishing at  $x=-1$ .
...
Setting this equal to the integrand:
 ...
 $$\frac{Q(P+P') - PQ'}{Q^2} = \frac{x^3 - 2}{(x+1)^2}.$$ 

Cross-multiplying:

 $$(x+1)^2\bigl(Q(P+P') - PQ'\bigr) = (x^3 - 2)Q^2. \qquad \text{(*)}$$ 

At  $x = -1$ :  $0 = (-3)Q(-1)^2$ , so  $Q(-1) = 0$ .

This means  $Q(-1) = 0$ , so  $(x + 1)$  divides  $Q(x)$ .

**Showing  $\deg P = \deg Q + 1$ :** Let  $\deg P = p$  and  $\deg Q = q$ . In  $Q(P+P') - PQ'$ , the term  $QP$  has degree  $p+q$ , while  $QP'$  and  $PQ'$  have degree at most  $p+q-1$ . Thus the bracket has degree  $p+q$ .

Comparing degrees in  $(*)$ :

 $$p+q+2 = 2q+3,$$ 

so  $p = q+1$ . Hence the degree of  $P$  is exactly one more than the degree of  $Q$ .

**Finding  $P(x)$  when  $Q(x) = x + 1$ :** Here  $q = 1$ , so  $p = 2$ . Write  $P(x) = ax^2 + bx + c$ .

Since  $\frac{d}{dx}\left[\frac{P}{Q}e^x\right] = \frac{x^3-2}{(x+1)^2}e^x$  with  $Q = x+1$ , we need:
...
Numerator:  $(x-2)(x^2+2x+1) + 3x + 3 - 3 = x^3 + 2x^2 + x - 2x^2 - 4x - 2 + 3x = x^3 - 2$ , as required.
...
Differentiating and clearing denominators as before gives
...
The left side of this equation,  $(PQ + P'Q - PQ')(x+1)$ , vanishes to order  $n$ .
...
Slightly less people try it than Q2 but the success rate is slightly higher. In both parts, the candidate successfully equated the derivative of the right-hand expression to the integrand (eliminating the exponential function). In the first part, many people solved $P(x)$ given $Q(x)$, but there were many illogical steps in proving that $\deg P$ is one greater than $\deg Q$ and that no such polynomial exists in (ii). Many people would be better off multiplying the denominator and using the remainder/factor theorem rather than trying an argument based on the degree of a rational expression.
...
**Topic**: Series and Sequences &nbsp;|&nbsp; **Difficulty**: Challenging &nbsp;|&nbsp; **Marks**: 20
...
The considered expression equates to  $\frac{(x-1)x^r}{(1+x^r)(1+x^{r+1})}$  and so, by the method of differences,  $\sum_{r=1}^N \frac{x^r}{(1+x^r)(1+x^{r+1})} = \frac{1}{(x-1)} \left[ \frac{1}{1+x} - \frac{1}{1+x^{N+1}} \right]$ , and letting  $N \to \infty$ , the desired result is obtained. Writing  $\operatorname{sech}(ry)$  as  $\frac{2e^{-ry}}{1+e^{-2ry}}$  and similarly  $\operatorname{sech}((r + 1)y)$ , the result of part (i) with  $x = e^{-2y}$  can be used to obtain the result. Care needs to be taken to write  $\sum_{r=-\infty}^{\infty} \operatorname{sech}(ry) \operatorname{sech}((r + 1)y)$  as  $2[\sum_{r=1}^{\infty} \operatorname{sech}(ry) \operatorname{sech}((r + 1)y) + \operatorname{sech} y]$  which with the previous deduction of (ii) can be simplified to  $2 \operatorname{cosech} y$ .
...
Slightly more people try it than Q2 (about four out of five), but the success rate is slightly lower. The first part (with explicit hints) is well done, but the $N$ limit argument going to infinity is poorly notated. In (ii), quite a few candidates wrote the expression of sech(ry) using positive exponential powers, resulting in the inability to apply the result of (i). Few candidates completely simplified the final result, and many failed to handle the sum of the positive and negative parts correctly, with some simply multiplying the sum from 1 to infinity by 2.
...
**Topic**: Combinatorics and Number Theory &nbsp;|&nbsp; **Difficulty**: Hard &nbsp;|&nbsp; **Marks**: 20
...
Since  $2m \geq 4$  is even and therefore not prime, increasing the upper bound from  $2m - 1$  to  $2m$  introduces no new prime factor. Hence:

 $$P_{1,2m} = P_{1,2m-1}.$$ 

Since  $2m - 1 \geq 3 \geq 2$ , the inductive hypothesis gives  $P_{1,2m-1} < 4^{2m-1}$ . Therefore:

 $$P_{1,2m} = P_{1,2m-1} < 4^{2m-1} < 4^{2m}.$$ 
...
About half of the candidates attempted this question and scored similar to Q2. The binomial expansion and its symmetries are well dealt with in the first part, and the results of (iii) can also be applied in (iv). But (ii) is poorly answered, because arguments about divisibility tend to be too arbitrary; in (iii), little attention is paid to the need for $m+1 \le 2m$ to use the result of (ii).
...
**Topic**: Hyperbolic Functions &nbsp;|&nbsp; **Difficulty**: Challenging &nbsp;|&nbsp; **Marks**: 20
...
**Case 3:  $-A < B < A$ .** We try  $R \sinh(x + \gamma) = R \sinh x \cosh \gamma + R \cosh x \sinh \gamma$ , matching  $A = R \cosh \gamma$  and  $B = R \sinh \gamma$ . Then  $\tanh \gamma = B/A$ , so  $\gamma = \operatorname{artanh} \frac{B}{A}$ , and  $R^2 = A^2 - B^2 > 0$ , giving  $R = \sqrt{A^2 - B^2}$ . This includes positive, zero and negative values of  $B$  in this range.
...
About 40% of the candidates attempted this question and scored about one-third of the score. Most know the cosh addition formula, but the A=B case is poorly handled. Part (i) is not explained clearly and the ± sign is misunderstood, resulting in (ii) false ±. (iii)(iv) The necessary and sufficient condition argument is weak. Only a few outstanding candidates answered it completely.
...
The least popular question in the pure mathematics section was attempted by less than 30%, resulting in poor overall performance. Most can complete the stem part, but verification of a numerical factor of 1 is often ignored. Despite having stem, most don't know what to do with (i) and therefore make little progress.
...
**Topic**: Functional Equations &nbsp;|&nbsp; **Difficulty**: Standard &nbsp;|&nbsp; **Marks**: 20
...
For  $x \neq 0$ , this gives  $f(x)=x$ . When  $x=0$ , the original equation gives  $f(0)+f(0)=0$ , so  $f(0)=0$ . Thus

 $$f(x) = x \qquad \text{for all } x. \qquad \text{(3)}$$ 
...
Now check (5):
 ...
 $$g(x) + x \, g\!\left(\frac{x+1}{x-1}\right) = \frac{2x}{x^2+1} + x \cdot \frac{x^2-1}{x^2+1} = \frac{2x + x^3 - x}{x^2+1} = \frac{x^3 + x}{x^2+1} = \frac{x(x^2+1)}{x^2+1} = x.$$ 
...
**Verification:**
 ...
 $$h(x) + h\!\left(\frac{1}{1-x}\right) = \left(\frac{1}{2} - x\right) + \left(\frac{1}{2} - \frac{1}{1-x}\right) = 1 - x - \frac{1}{1-x}.$$ 
...
The number of attempts was close to Q1 (the most), and it was slightly more successful, with many getting perfect scores. The idea of ​​function iteration constructing loops is commonly found. There was difficulty in finding g(x) in (ii). Some candidates substituted the known g(x) instead of solving it. (iii) Some candidates only made the first substitution and then stopped, unable to find the solution. There are also some answers that are partially guessed but not fully demonstrated.
