...
|---|-------|------------|----------------|
| 1 | Calculus and Series (Calculus and Series) | Standard | Derivation to determine monotonicity, summation of split terms using logarithmic differences, Taylor expansion of inequalities |
| 2 | Trigonometry and Geometry | Standard | Sine theorem, sum angle/multiple angle formula, coordinates and distance calculations in triangles |
| 3 | Combinatorics and Sequences | Challenging | Triangle inequalities, categorical counting, mathematical induction, using recursion relations |
| 4 | Functions and Graphs | Standard | Inverse function and composite function image transformation, stationary point analysis, asymptote judgment, periodic translation |
| 5 | Trigonometric Identities and Series | Challenging | Mathematical induction, tan addition formula, discussion of the value range of arctan, inverse trigonometric function series |
| 6 | Integration | Challenging | Proof of trigonometric identities, substitution method of definite integrals, use of symmetry to simplify integrals, idea of integral by parts |
| 7 | Analytic Geometry | Challenging | Equation of circle, Pythagorean theorem, distance formula, derivation of inequality, y²≥0 condition |
| 8 | Vector Geometry (Vector Geometry) | Challenging | Vector linear combination, similar triangle proportions, collinearity determination (vectors parallel), midpoint conditions |

---

## Question 1

**Topic**: Calculus and Series &nbsp;|&nbsp; **Difficulty**: Standard &nbsp;|&nbsp; **Marks**: 20
...
For the first result, show that the gradient of the function is positive for all positive values of  $x$  (by differentiating) and also that  $f(0) \ge 0$ . Once this result has been established sum a set of the terms, using  $x = \frac{1}{k}$ , note that  $\ln(1 + \frac{1}{k})$  can be written as  $\ln(k + 1) - \ln(k)$  and then the required result follows.

For the second part, first show that  $x + \ln(1 - x)$  is *negative* for  $0 < x < 1$  and then use the substitution  $x = \frac{1}{k^2}$ , noting that  $\ln(1 - \frac{1}{k^2})$  can be written as  $\ln(k - 1) - 2\ln(k) + \ln(k + 1)$ . Deal with the sum starting with  $k = 2$  and then add the initial 1 afterwards.
...
Popular questions but average score is low. Common mistakes: (1) Failure to verify that the gradient is positive and f(0)≥0 at the same time; (2) Failure to use the second part of the results as required and using the graphical method; (3) The sign of the chain rule is wrong; (4) The summation formula is written incorrectly.
...
**Topic**: Trigonometry and Geometry &nbsp;|&nbsp; **Difficulty**: Standard &nbsp;|&nbsp; **Marks**: 20
...
As with all geometric questions a good diagram of the information given makes the solution to this question much easier to reach. The first result in this question follows from an application of the sine rule with applications of the relevant formulae for  $\sin(A + B)$  and the double angle formulae. From a diagram of the triangle it should then be an easy application of trigonometry to show that  $DE = \frac{x}{2}$ . There are a number of different methods for establishing that  $FC$  trisects the angle  $ACB$  - one method is to show that  $\sin(\angle FCE) = \frac{1}{2}$ , following which it is relatively straightforward to work out the sizes of angles  $ACB$  and  $ACF$  in terms of  $\alpha$  and show that they must satisfy the correct relationship.
...
The highest average score. There are various solutions but the explanations are often not clear enough; it is recommended to draw a large picture and be careful not to invert the angle markings in the picture.
...
**Topic**: Combinatorics and Sequences &nbsp;|&nbsp; **Difficulty**: Challenging &nbsp;|&nbsp; **Marks**: 20
...
The overall performance is good, and there are many people with perfect scores. Frequently asked questions: (1) $T_8-T_7$ is not understood as the number of triangles containing specific poles and is enumerated one by one; (2) the basic cases of the induction method are not exhaustively verified; (3) the induction method is not tried or used incorrectly in odd cases.
...
**Topic**: Functions and Graphs &nbsp;|&nbsp; **Difficulty**: Standard &nbsp;|&nbsp; **Marks**: 20
...
Generally performs well. Common mistakes: (1) Draw discontinuous images (despite claiming to be continuous); (2) Label points on the image incorrectly; (3) Make mistakes when reasoning with image transformations; (4) Do integrals instead of differentials.
...
**Topic**: Trigonometric Identities and Series &nbsp;|&nbsp; **Difficulty**: Challenging &nbsp;|&nbsp; **Marks**: 20
...
The initial proof by induction is a straightforward application of the  $\tan(A + B)$  formula. The final part of section (i) requires recognition that there are many possible values of  $x$  to give a particular value of  $\tan x$ , but only one of them is the value that would be obtained by applying the  $\arctan$  function. The result can therefore be shown by establishing that the difference between consecutive terms of the sequence is never more than  $\pi$ .

For the second part of the question a diagram of the triangle and application of the  $\tan 2A$  formula shows that the value of  $\alpha_n$  must be of the form used in the first part of the question. All that remains is then to show that the limit of the sum must give the required value.
...
The average score is low. Most people can complete the inductive proof $\tan S_n = n/(n+1)$, but most people do not realize that further proof $S_n = \arctan(n/(n+1))$ is needed (need to discuss the range). The second part performs better if it identifies a connection with the first part.
...
**Topic**: Analytic Geometry &nbsp;|&nbsp; **Difficulty**: Challenging &nbsp;|&nbsp; **Marks**: 20
...
**Topic**: Vector Geometry &nbsp;|&nbsp; **Difficulty**: Challenging &nbsp;|&nbsp; **Marks**: 20
