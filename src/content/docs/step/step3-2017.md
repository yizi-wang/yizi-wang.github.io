...
|---|-------|------------|----------------|
| 1 | Combinatorics and Series (Combinatorics and Series) | Standard | telescoping series, binomial coefficient expansion, inequality scaling, series summation |
| 2 | Complex Transformations | Hard | Complex exponential rotation formula, transformation compound, trigonometric and complex number form conversion, classification discussion |
| 3 | Algebra (Algebra) | Challenging | Pre-solve cubic equations, Vieta formula, symmetric functions, finding roots of quadratic equations |
| 4 | Calculus (Calculus) | Challenging | Changing bases of logarithms, integral and exponential operations, separation of variables in differential equations, proof by contradiction |
| 5 | Calculus (Calculus) | Challenging | Polar coordinate derivatives, derivation of orthogonal conditions, separation of variables method, cardioid line drawing |
| 6 | Integration and Substitution | Challenging | Integration with substitution (u=1/v and Möbius transformation), derivation of functional equations, recursion and elimination by substitution |
| 7 | Analytic Geometry: Ellipse and Tangents | Challenging | Substitution of parametric equations, derivation of implicit functions, derivation of tangent equations, discriminants and Vedic theorem |
| 8 | Summation and Trigonometric Identities | Challenging | Abel summation, trigonometric identities (integration and difference), split term summation skills |

---

## Question 1

**Topic**: Combinatorics and Series &nbsp;|&nbsp; **Difficulty**: Standard &nbsp;|&nbsp; **Marks**: 20
...
(ii) First inequality:
  ...
  $${}^{n+1}C_3 = \frac{n^3-n}{3!} < \frac{n^3}{3!}$$  

so   $\frac{3!}{n^3} < \frac{1}{{}^{n+1}C_3}$  .
...
Since   ${}^{N+r}C_r \to \infty$   as   $N \to \infty$  , the second term vanishes:
  ...
  $$\sum_{n=1}^{\infty} \frac{1}{{}^{n+r}C_{r+1}} = \frac{r+1}{r}$$  
...
For   $n \geq 3$  , the denominator is positive, so this expression is negative:
  ...
  $$\frac{20}{{}^{n+1}C_3} - \frac{1}{{}^{n+2}C_5} < \frac{5!}{n^3}$$  
...
Also   $\sum_{n=3}^{\infty} \frac{1}{n^3} = \sum_{n=1}^{\infty} \frac{1}{n^3} - 1 - \frac{1}{8}$  , so
  ...
  $$\sum_{n=1}^{\infty} \frac{1}{n^3} < \frac{9}{8} + \frac{1}{12} = \frac{27+2}{24} = \frac{116}{96}$$  
...
Substituting:
  ...
  $$\sum_{n=1}^{\infty} \frac{1}{n^3} > \frac{9}{8} + \frac{35}{480} = \frac{108}{96} + \frac{7}{96} = \frac{115}{96}$$  
...
**Topic**: Complex Transformations &nbsp;|&nbsp; **Difficulty**: Hard &nbsp;|&nbsp; **Marks**: 20
...
This is a **translation** by   $(a - b)(e^{i\phi} - 1)$  .
...
As a check,   $(1+i)(1-i)(-1+2i)(-1-2i) = 2 \cdot 5 = 10 = s$  .
...
(iv) Setting   $g(y)=\ln f(y)$  , the condition   $F(y)=\sqrt{f(y)}$   gives

  $$\int_0^y g(x)\,dx = \frac{y}{2}g(y).$$  

Differentiating gives   $g(y)=yg'(y)$  , so   $\left(\frac{g(y)}{y}\right)'=0$  .

Hence   $g(y)=ky$  , giving   $f(y)=e^{ky}=b^y$  .
...
This is consistent with part (iii), where the geometric mean of   $b^x$   is   $\sqrt{b^y} = \sqrt{f(y)}$  .
...
All three cardioids pass through the origin. The first two are nested (same orientation, different sizes) with cusps at the bottom and lobes extending upward. The third has its cusp at the top and its lobe extending downward, crossing the first two at right angles at every intersection.
...
**Topic**: Integration and Substitution &nbsp;|&nbsp; **Difficulty**: Challenging &nbsp;|&nbsp; **Marks**: 20
...
**Topic**: Analytic Geometry: Ellipse and Tangents &nbsp;|&nbsp; **Difficulty**: Challenging &nbsp;|&nbsp; **Marks**: 20

### Problem

**7** Show that the point   $T$   with coordinates
  $$ \left( \frac{a(1 - t^2)}{1 + t^2} , \frac{2bt}{1 + t^2} \right) \qquad \text{(*)} $$  
...
Expanding the right side gives   $-b(X+a)t^4 + 2aYt^3 - b(X+a)t^2 - b(a-X)t^2 + 2aYt - b(a-X)$  , and   $-b(X+a)t^2 - b(a-X)t^2 = -b[(X+a)+(a-X)]t^2 = -2abt^2$  , as required.
...
**Topic**: Summation and Trigonometric Identities &nbsp;|&nbsp; **Difficulty**: Challenging &nbsp;|&nbsp; **Marks**: 20
...
(i) By setting   $b_m = \sin mx$  , show that
  $$ \sum_{m=1}^{n} \cos\bigl((m + \tfrac{1}{2})x\bigr) = \tfrac{1}{2} \big( \sin(n + 1)x - \sin x \big) \operatorname{cosec} \tfrac{1}{2}x . $$  
...
Using the note:   $\sin(m+1)x - \sin mx = 2\cos\bigl((m+\tfrac{1}{2})x\bigr) \sin\tfrac{1}{2}x$  .

RHS:   $a_{n+1}b_{n+1} - a_1b_1 = \sin(n+1)x - \sin x$  , and   $\sum b_{m+1}(a_{m+1}-a_m) = 0$   since   $a_m$   is constant.

So   $2\sin\tfrac{1}{2}x \sum_{m=1}^{n} \cos\bigl((m+\tfrac{1}{2})x\bigr) = \sin(n+1)x - \sin x$  , giving   $\sum_{m=1}^{n} \cos\bigl((m+\tfrac{1}{2})x\bigr) = \tfrac{1}{2}(\sin(n+1)x - \sin x)\csc\tfrac{1}{2}x$  .
...
Thus   $\sum_{m=1}^{n} m\sin mx = (p\sin(n+1)x + q\sin nx)\csc^2\tfrac{1}{2}x$  
...
By the given identity   $\sin A - \sin B = 2\cos\!\left(\frac{A+B}{2}\right)\sin\!\left(\frac{A-B}{2}\right)$  :
  ...
  $$\sin(m+1)x - \sin mx = 2\cos\!\left(\frac{(m+1)x + mx}{2}\right)\sin\!\left(\frac{(m+1)x - mx}{2}\right) = 2\cos\!\left((m+\tfrac{1}{2})x\right) \cdot \sin\tfrac{1}{2}x$$  

Substituting into   $(*)$  :

  $$\sum_{m=1}^{n} 1 \cdot 2\cos\!\left((m+\tfrac{1}{2})x\right) \cdot \sin\tfrac{1}{2}x = \sin(n+1)x - \sin x$$  

  $$2\sin\tfrac{1}{2}x \sum_{m=1}^{n} \cos\!\left((m+\tfrac{1}{2})x\right) = \sin(n+1)x - \sin x$$  

  $$\sum_{m=1}^{n} \cos\!\left((m+\tfrac{1}{2})x\right) = \frac{\sin(n+1)x - \sin x}{2\sin\tfrac{1}{2}x} = \tfrac{1}{2}\bigl(\sin(n+1)x - \sin x\bigr)\operatorname{cosec}\tfrac{1}{2}x$$  
