...
|---|-------|------------|----------------|
| 1 | Integration | Challenging | Universal substitution, recurrence relations, partial fraction decomposition |
| 2 | Series Series | Hard | Leibniz's law, Maclaurin expansion, differential equation recursion |
| 3 | Vectors and Geometry | Challenging | Vector dot product, coordinate geometry, symmetry |
| 4 | Complex Numbers | Challenging | Unit root factorization, complex number substitution, trigonometric identities |
| 5 | Number Theory | Hard | Co-prime argument, prime factor analysis, proof by contradiction |
| 6 | Complex Numbers | Challenging | Expansion of complex modules, simplification of conjugate products, triangle inequalities, derivation of inequalities |
| 7 | Differential Equations | Hard | Energy function construction, first integration, differential verification, inequality estimation |
| 8 | Complex Numbers and Geometry | Hard | Sum of geometric series, properties of conic focus directrix, trigonometric identities, complex number method |
...
Use the tangent half-angle substitution exactly as given: express  $dx$  and  $\sin x$  in terms of  $t$ , then complete the square in  $t^2+2at+1$ . For  $I_3$ , write  $\sin^{n+1}x=\sin^n x(2+\sin x)-2\sin^n x$  to obtain a recurrence for  $I_{n+1}+2I_n$ , and start from  $I_0$  by taking  $a=\frac12$  in the first integral.
...
**Topic**: Series Series &nbsp;|&nbsp; **Difficulty**: Hard &nbsp;|&nbsp; **Marks**: 20
...
Equivalently, using  $(\star)$  directly with  $x=0$  gives  $y^{(n+2)}(0) = (n+1)^2 y^{(n)}(0)$ , and hence

 $$a_{n+2} = \frac{(n+1)^2}{(n+2)!} y^{(n)}(0) = \frac{(n+1)^2}{(n+2)!} \cdot n! \cdot a_n = \frac{n+1}{n+2} a_n.$$ 
...
For  $n\geqslant 1$ ,  $2^2 \times 3^2 \times \cdots \times n^2 = (2\cdot3\cdots n)^2=(n!)^2$ , since the omitted factor  $1^2$  is  $1$ . This also matches the first term if we take  $n=0$ , since  $\frac{(0!)^2}{1!}=1$ .
...
**Topic**: Vectors and Geometry **Difficulty**: Challenging **Marks**: 20
...
**Topic**: Number Theory Number Theory &nbsp;|&nbsp; **Difficulty**: Hard &nbsp;|&nbsp; **Marks**: 20
...
5. Writing  $q^nN=p^n$  and using the given divisibility result with  $\gcd(p^n,q^n)=1$  shows directly that  $p^n$  divides  $N$ . Thus  $N=kp^n$ , and substituting back gives  $q^nk=1$ , so  $q=1$  and  $k=1$ . For part (ii), first show that  $b^a$  divides  $d^b$  and  $a^a$  divides  $c^b$ , then use  $\gcd(c,d)=1$  to force the common multiplier to be  $1$ . Comparing highest powers of a prime  $p$  in  $d^b=b^a$  gives  $mb=na$ , and the coprimality of  $a$  and  $b$  then leads to  $p^n\leq n$ .
...
**Topic**: Differential Equations Differential Equations &nbsp;|&nbsp; **Difficulty**: Hard &nbsp;|&nbsp; **Marks**: 20
...
**Topic**: Complex Numbers and Geometry &nbsp;|&nbsp; **Difficulty**: Hard &nbsp;|&nbsp; **Marks**: 20

### Problem

**8** Evaluate  $\sum_{r=0}^{n-1} \mathrm{e}^{2\mathrm{i}(\alpha+r\pi/n)}$  where  $\alpha$  is a fixed angle and  $n \geqslant 2$ .

The fixed point  $O$  is a distance  $d$  from a fixed line  $D$ . For any point  $P$ , let  $s$  be the distance from  $P$  to  $D$  and let  $r$  be the distance from  $P$  to  $O$ . Write down an expression for  $s$  in terms of  $d$ ,  $r$  and the angle  $\theta$  between  $OP$  and the perpendicular from  $O$  to  $D$ .


The curve  $E$  is such that, for any point  $P$  on  $E$ , the relation  $r = ks$  holds, where  $k$  is a fixed number with  $0 < k < 1$ .
