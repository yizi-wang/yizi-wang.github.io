...
Comparing with the $\arcsin$ result, set $a = 1$, $b = 3$. Then $\sqrt{b-a} = \sqrt{2}$ and:

$$\frac{\mathrm{d}}{\mathrm{d}x}\arcsin\!\left(\frac{x+1}{x+3}\right) = \frac{\sqrt{2}}{(x+3)\sqrt{4+2x}} = \frac{\sqrt{2}}{(x+3)\sqrt{2}\sqrt{x+2}} = \frac{1}{(x+3)\sqrt{x+2}}.$$

This gives $\frac{1}{(x+3)\sqrt{x+2}}$, not quite what we need. Let us try the $\operatorname{arcosh}$ result instead with a suitable choice.

Set $a = 1$, $b = 3$ in the $\operatorname{arcosh}$ derivative:

$$\frac{\mathrm{d}}{\mathrm{d}x}\operatorname{arcosh}\!\left(\frac{x+3}{x+1}\right) = \frac{-\sqrt{2}}{(x+1)\sqrt{4+2x}} = \frac{-\sqrt{2}}{(x+1)\cdot\sqrt{2}\sqrt{2+x}} = \frac{-1}{(x+1)\sqrt{x+2}}.$$

This gives $\frac{1}{(x+1)\sqrt{x+2}}$, still not matching. We need $\frac{1}{(x+1)\sqrt{x+3}}$.

Let us instead use the $\arcsin$ result directly. Set $a = 1$, $b = 3$:

$$\frac{\mathrm{d}}{\mathrm{d}x}\arcsin\!\left(\frac{x+1}{x+3}\right) = \frac{\sqrt{2}}{(x+3)\sqrt{2x+4}} = \frac{1}{(x+3)\sqrt{x+2}}.$$

Not matching. Let us try $a = 1$, $b = 5$ in $\arcsin$:

$$\frac{\mathrm{d}}{\mathrm{d}x}\arcsin\!\left(\frac{x+1}{x+5}\right) = \frac{2}{(x+5)\sqrt{2x+6}} = \frac{2}{(x+5)\sqrt{2}\sqrt{x+3}} = \frac{\sqrt{2}}{(x+5)\sqrt{x+3}}.$$

Not matching either. Let us try $a = 3$, $b = 5$ in $\arcsin$:

$$\frac{\mathrm{d}}{\mathrm{d}x}\arcsin\!\left(\frac{x+3}{x+5}\right) = \frac{\sqrt{2}}{(x+5)\sqrt{2x+8}} = \frac{\sqrt{2}}{(x+5)\cdot 2\sqrt{x+2}} = \frac{1}{\sqrt{2}(x+5)\sqrt{x+2}}.$$

Let us try a substitution approach instead. For integral (i), let $u = \sqrt{x+3}$, so $x = u^2 - 3$, $\mathrm{d}x = 2u\,\mathrm{d}u$:

$$\int \frac{1}{(x+1)\sqrt{x+3}}\,\mathrm{d}x = \int \frac{2u}{(u^2-2)\cdot u}\,\mathrm{d}u = \int \frac{2}{u^2-2}\,\mathrm{d}u.$$

For $x > -1$, we have $u > \sqrt{2}$, so $u^2 > 2$:

$$\int \frac{2}{u^2-2}\,\mathrm{d}u = \frac{2}{2\sqrt{2}}\ln\left|\frac{u-\sqrt{2}}{u+\sqrt{2}}\right| = \frac{1}{\sqrt{2}}\ln\frac{\sqrt{x+3}-\sqrt{2}}{\sqrt{x+3}+\sqrt{2}}.$$

Or equivalently, using the $\operatorname{arcosh}$ result. Setting $a = 1$, $b = 1$ gives $b-a = 0$ which is degenerate. Let us relate this to the $\operatorname{arcosh}$ derivative.

From the $\operatorname{arcosh}$ result with $a = -1$, $b = 3$ (noting $x + a = x - 1$ which requires $x > 1$):

$$\frac{\mathrm{d}}{\mathrm{d}x}\operatorname{arcosh}\!\left(\frac{x+3}{x-1}\right) = \frac{-2}{(x-1)\sqrt{2x+2}}.$$

This doesn't quite work for our domain. The substitution method gives us the cleanest answer.

Therefore:

$$\int \frac{1}{(x+1)\sqrt{x+3}}\,\mathrm{d}x = \frac{1}{\sqrt{2}}\ln\frac{\sqrt{x+3}-\sqrt{2}}{\sqrt{x+3}+\sqrt{2}} + C.$$

**Step 4: Integral (ii).**

We need $\int \frac{1}{(x+3)\sqrt{x+1}}\,\mathrm{d}x$ for $x > -1$.

Let $u = \sqrt{x+1}$, so $x = u^2 - 1$, $\mathrm{d}x = 2u\,\mathrm{d}u$:
