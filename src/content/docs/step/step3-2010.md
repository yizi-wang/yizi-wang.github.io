...
|---|-------|------------|----------------|
| 1 | Statistics (Statistics) | Standard | Transformations of summation operations, algebraic simplification, proof of inequality |
| 2 | Integration | Challenging | Partial fraction decomposition, hyperbolic function identity, anomalous integral calculation |
| 3 | Complex Numbers and Polynomials | Challenging | Properties of unit roots, construction of cyclotomic polynomials, factorization of polynomials, proof by contradiction |
| 4 | Polynomials | Standard | Common root elimination method, knot theory, algebraic transformations |
| 5 | Coordinate Geometry (Coordinate Geometry) | Standard | Coordinate method, straight line intersection calculation, vertical condition verification, ruler drawing |
| 6 | 3D Rotations and Coordinate Geometry (3D Rotations and Coordinate Geometry) | Hard | Rotation matrix, coordinate transformation, vector cross product, trigonometric identity |
| 7 | Differential Equations and Series Expansion (Differential Equations and Series Expansion) | Hard | Derivation of implicit functions, mathematical induction, power series expansion, trigonometric polynomials |
| 8 | Integration Techniques and Differential Equations (Integration Techniques and Differential Equations) | Challenging | Inverse application of derivation rules for quotients, undetermined coefficient method, integral factor method |

---

## Question 1

**Topic**: Statistics &nbsp;|&nbsp; **Difficulty**: Standard &nbsp;|&nbsp; **Marks**: 20
...
Simplifying:
 ...
 $$\frac{1 - e^{-a}}{1 + e^a} = e^{-a}\frac{e^a - 1}{e^a + 1} = e^{-a}\tanh\frac{a}{2}.$$ 

Therefore the integral equals:

 $$\frac{1}{2\cosh a}\left[0 - \ln\left(e^{-a}\tanh\frac{a}{2}\right)\right] = \frac{1}{2\cosh a}\left(a+\ln\left(\coth\frac{a}{2}\right)\right).$$ 
...
**Topic**: Complex Numbers and Polynomials &nbsp;|&nbsp; **Difficulty**: Challenging &nbsp;|&nbsp; **Marks**: 20
...
**Proof that roots are disjoint**: Suppose  $\alpha$  is both a primitive  $n$ th root and a primitive  $m$ th root. If  $n<m$ , then  $\alpha^n=1$  with  $0<n<m$ , contradicting the fact that  $\alpha$  is a primitive  $m$ th root. The case  $m<n$  is symmetric. Hence no root can occur in two different  $C_n$ .
...
**Topic**: Polynomials &nbsp;|&nbsp; **Difficulty**: Standard &nbsp;|&nbsp; **Marks**: 20
...
**Topic**: Coordinate Geometry &nbsp;|&nbsp; **Difficulty**: Standard &nbsp;|&nbsp; **Marks**: 20
...
The line through  $C(a,a)$  and  $P(an, 0)$  has direction  $(a(n-1),-a)$ .
...
6. By the result above, the line  $AT$  is perpendicular to the diagonal  $AC$ .
7. Rotate the labelling through a right angle and repeat the same construction at  $B$ ,  $C$  and  $D$ . This constructs the four lines through the vertices perpendicular to the corresponding diagonals.
8. These four lines enclose a square. In coordinates, they are parallel to  $y=-x$ ,  $y=x-a$ ,  $y=-x+2a$  and  $y=x+a$ , so adjacent vertices are separated by distance  $a\sqrt{2}$ . Hence the constructed square has area  $(a\sqrt{2})^2=2a^2$ .
...
**Topic**: 3D Rotations and Coordinate Geometry &nbsp;|&nbsp; **Difficulty**: Hard &nbsp;|&nbsp; **Marks**: 20
...
Thus  $R_2 = (-\cos\phi\sin\lambda, -\sin\phi\sin\lambda, \cos\lambda)$ .
...
**Topic**: Differential Equations and Series Expansion &nbsp;|&nbsp; **Difficulty**: Hard &nbsp;|&nbsp; **Marks**: 20
...
**Topic**: Integration Techniques and Differential Equations &nbsp;|&nbsp; **Difficulty**: Challenging &nbsp;|&nbsp; **Marks**: 20
...
Substitute  $R(x) = a + b\sin x + c\cos x$  and  $Q(x) = 1 + \cos x + 2\sin x$  directly into  $P = QR' - Q'R$  and match coefficients of  $1$ ,  $\sin x$ ,  $\cos x$ .
