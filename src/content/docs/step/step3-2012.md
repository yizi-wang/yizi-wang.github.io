...
|---|-------|------------|----------------|
| 1 | Differential Equations | Challenging | Chain rule, variable substitution and order reduction, initial value problem solving |
| 2 | Infinite Series and Products | Challenging | Split term elimination, logarithmic differentiation, infinite products to series, partial fraction decomposition |
| 3 | Coordinate Geometry and Curve Analysis | Challenging | Curve drawing, simultaneous equations, factorization, parameter classification discussion |
| 4 | Infinite Series Summation | Challenging | Taylor expansion, factorial series summation, partial fraction decomposition, split term elimination |
| 5 | Number Theory and Rational Points | Challenging | Trigonometric substitution, parameterization of rational points, quadratic domain extension, constructive proof |
| 6 | Complex Numbers and Complex Plane Complex Numbers and Argand Diagram | Standard | Separation of real and imaginary parts of complex numbers, parameter elimination method, root locus analysis, Argand diagram construction |
| 7 | Differential Equations | Challenging | High-order ODE of equations, characteristic equation method, initial value and boundary value solution, infinite series summation |
| 8 | Sequences and Series | Challenging | Fibonacci recursion relationship, odd and even discussion, arctangent addition formula, telescoping series |

---

## Question 1

**Topic**: Differential Equations Differential Equations &nbsp;|&nbsp; **Difficulty**: Challenging &nbsp;|&nbsp; **Marks**: 20
...
Use the displayed derivative formula as a first integral. For part (i), take  $n=1$ , so  $z=y(y')^2$ , and use the differential equation to express  $\frac{dz}{dx}$  in terms of  $z$ . The initial condition fixes the constant. For part (ii), set  $p=\frac{dy}{dx}$  and regard  $p$  as a function of  $y$ , so  $y''=p\frac{dp}{dy}$ ; then use the substitution  $p=yv$ .
...
Since  $z = y(y')^2$ , we can write  $\frac{dz}{dx} = y'\sqrt{y}$ . Also  $y' = \pm\sqrt{z/y}$ , so:

 $$\frac{dz}{dx} = \pm\frac{\sqrt{z}}{\sqrt{y}} \cdot \sqrt{y} = \pm\sqrt{z}$$ 

Taking the positive branch for  $x \geq 0$  gives:
...
Let  $p = \frac{dy}{dx}$  and regard  $p$  as a function of  $y$ . The equation is  $p^2 - yp\frac{dp}{dy} + y^2 = 0$  (using  $y'' = p\frac{dp}{dy}$ ).
...
**Verification:**  $y' = xe^{x^2/2}$ ,  $y'' = (1 + x^2)e^{x^2/2}$ .
 ...
 $(y')^2 - yy'' + y^2 = x^2 e^{x^2} - e^{x^2/2}(1+x^2)e^{x^2/2} + e^{x^2} = x^2 e^{x^2} - (1+x^2)e^{x^2} + e^{x^2} = 0$ .
...
**Topic**: Infinite Series and Products &nbsp;|&nbsp; **Difficulty**: Challenging &nbsp;|&nbsp; **Marks**: 20
...
**Second deduction:** From  $(1-x)\prod_{r=0}^{n}(1+x^{2^r}) = 1 - x^{2^{n+1}}$ , taking logarithms gives:
...
Using  $1 + x^{3\cdot 2^r} = (1+x^{2^r})(1 - x^{2^r} + x^{2^{r+1}})$  (sum of cubes):
...
Differentiate  $\ln(1+x+x^2) = -\sum_{r=0}^{\infty}\ln(1-x^{2^r}+x^{2^{r+1}})$ .
...
**Topic**: Coordinate Geometry and Curve Analysis Coordinate Geometry and Curve Analysis &nbsp;|&nbsp; **Difficulty**: Challenging &nbsp;|&nbsp; **Marks**: 20
...
The cases that arise are  $a = 1, k = 21, \frac{k}{m} = \frac{21}{12} < 2$  (d),  $a = \frac{-1+\sqrt{13}}{2}, k = \frac{13}{2}\sqrt{13} - \frac{5}{2}, \frac{k}{m} < 2$  (d), and  $a = \frac{-1-\sqrt{13}}{2}, k = \frac{-13}{2}\sqrt{13} - \frac{5}{2}, k < 0$  (a).
...
**Topic**: Infinite Series Summation &nbsp;|&nbsp; **Difficulty**: Challenging &nbsp;|&nbsp; **Marks**: 20
...
**Topic**: Number Theory and Rational Points **Difficulty**: Challenging **Marks**: 20
...
For rational points on  $x^2+y^2=1$ , use a Pythagorean triple such as  $3,4,5$ . For 2-rational points, start from an obvious integer 2-rational point and rotate it by rational sine and cosine values; this preserves  $x^2+y^2$  and can make all four rational coefficients non-integral. For the hyperbola, use the corresponding hyperbolic rotation with rational  $\cosh t$  and  $\sinh t$ .
...
Check:  $\left(\dfrac{7}{5}\right)^2 + \left(\dfrac{1}{5}\right)^2 = \dfrac{49}{25} + \dfrac{1}{25} = 2$ .

A non-integer rational point is  $\left(\dfrac{7}{5}, \dfrac{1}{5}\right)$ .

**Part (ii)(a)**

An integer 2-rational point on  $x^2 + y^2 = 3$  is  $(\sqrt{2},1)$ , corresponding to  $p=0$ ,  $q=1$ ,  $r=1$ ,  $s=0$ . Indeed,  $2+1=3$ .
...
Here  $p = \dfrac{4}{5}, q = \dfrac{3}{5}, r = \dfrac{3}{5}, s = -\dfrac{4}{5}$ , which are all non-integers.
...
Take  $p = 0, q = 1, r = 3, s = 0$ :  $0 + 2 + 9 + 0 = 11$ ,  $pq + rs = 0 + 0 = 0$ .
...
Take  $p = 3, q = 0, r = 0, s = 1$ :  $9 - 2 = 7$ ,  $pq - rs = 0 - 0 = 0$ .
...
**Topic**: Complex Numbers and Argand Diagram &nbsp;|&nbsp; **Difficulty**: Standard &nbsp;|&nbsp; **Marks**: 20
...
6. Substituting  $x + iy$  for  $z$  in the quadratic equation and equating real and imaginary parts yields the first two results, the imaginary gives two situations, one as required and the other substituted into the real gives the second. The first of these two results substituted into the real gives a circle radius 1, centre the origin, whilst the second gives the real axis without the origin. The second quadratic equation succumbs to the same approach giving the real axis without the origin (again), and a circle centre  $(-1,0)$  radius 1 also omitting the origin. The same approach in the third case yields the real axis with  $p = \frac{-x^2 \pm \sqrt{x^4 - 8x}}{2x}$  and, considering the discriminant,  $x < 0$  or  $x \geq 2$ .
...
We verify with the  $e^{-6t}$  coefficient:
 ...
 $$\frac{c(e - 1)e^5}{e^5 - 1} = \frac{2e(e^5 - 1)}{(e - 1)(e^6 - 1)} \cdot \frac{(e - 1)e^5}{e^5 - 1} = \frac{2e \cdot e^5}{e^6 - 1} = \frac{2e^6}{e^6 - 1}.$$ 
...
**Topic**: Sequences and Series &nbsp;|&nbsp; **Difficulty**: Challenging &nbsp;|&nbsp; **Marks**: 20
...
The first identity follows by substituting the small Fibonacci values. For part (ii), compare consecutive values of  $F_nF_{n+3}-F_{n+1}F_{n+2}$  and use the recurrence to show the sign alternates. For part (iii), apply the tangent addition formula to  $\arctan(1/F_{2r+1})+\arctan(1/F_{2r+2})$  and use the result of part (ii) to obtain a telescoping sum.
