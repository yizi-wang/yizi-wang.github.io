...
|---|-------|------------|----------------|
| 1 | Pure numbers | Challenging | Derivatives of parametric equations, simplification of trigonometric identities, distance formula from point to straight line, envelope method |
| 2 | Pure numbers | Hard | Matrix null space and determinant, symmetric polynomial identities, inequality proof, AM-GM inequality |
| 3 | Pure numbers | Challenging | Establishment of recursion relations, proof of integral inequality, handling of strict inequality signs |
| 4 | Pure numbers | Challenging | Vector projection formula, dot product properties, angle bisector conditions, proof of necessary and sufficient conditions |
| 5 | Pure numbers | Challenging | Simultaneous polar coordinate equations, discriminants to find tangent conditions, curve drawing skills, symmetry analysis |
| 6 | Pure numbers | Challenging | Identities of inverse trigonometric functions, derivation of hyperbolic functions, graphing of piecewise functions, analysis of discontinuities and asymptotes |
| 7 | Pure numbers | Hard | Complex number simplification, pure imaginary number discrimination, orthocentric properties, midpoint analysis, translation transformation |
| 8 | Pure numbers | Hard | Mathematical induction, inequality scaling, divergence discrimination, recursive sequence construction |
...
The most popular question (answered by 93% of candidates), with an average score of approximately 15/20. The examiner's report pointed out: (i) Most candidates can correctly calculate the normal equation, but many forget the negative sign on the normal gradient and only take the reciprocal; both implicit function derivation and parameter derivation are possible, and parameter derivation is more concise. (ii) About 3/4 of the candidates first find the intersection point of the normal line and the perpendicular line passing through the origin and then calculate the distance, but it is simpler to directly use the point-to-line distance formula; common mistakes include ignoring the absolute value sign and calculating the distance to the point on the curve instead of the distance to the normal line; the last question about the envelope equation is ignored by many candidates.
...
The fourth most popular question (about 80% responded), with an average score of about 9/20, and few people got full marks. The examiner's report emphasizes: (1) After declaring det(M)=0, you must demonstrate why it is true (the existence of non-trivial zero vectors); (2) When inequalities involve fractions, you must pay attention to the positive and negative properties; (3) The last question, x+y+z>2, can be proved in various ways (direct expansion, AM-GM, and case-by-case discussion are all acceptable). Ignoring hence requirements and doing pure algebra calculations is a common reason for deducting points.
...
The second most popular (84% responded), with an average score of just under 8/20. The examiner's report pointed out that: (1) When proving an inequality, it is more effective to prove that the proof is greater than zero than direct comparison, that is, consider (1/2)(I_{n+1}+I_{n-1})-I_n; (2) Common errors: insufficient demonstration of the positive and negative of integrals, wrong direction of logical derivation, weak inequality instead of strict inequality, inequality and variable range contradiction; (3) Some candidates ignore the condition of sec x cos beta <= 1 in (ii).
...
(i)
 ...
 $\boldsymbol{m.a} = \frac{1}{2}(\boldsymbol{a} + \boldsymbol{b}).\boldsymbol{a} = \frac{1}{2}(1 + \boldsymbol{a.b}) = m \cos \lambda$  where  $\lambda$  is the non-reflex angle between **a** and **m**
 $\boldsymbol{m.b} = \frac{1}{2}(\boldsymbol{a} + \boldsymbol{b}).\boldsymbol{b} = \frac{1}{2}(1 + \boldsymbol{a.b}) = m \cos \mu$  where  $\mu$  is the non-reflex angle between **b** and **m**
...
(ii)  $\boldsymbol{a_1.c} = (\boldsymbol{a} - (\boldsymbol{a.c})\boldsymbol{c}).\boldsymbol{c} = \boldsymbol{a.c} - \boldsymbol{a.c} \ \boldsymbol{c.c} = 0$  as required. *B1
...
but also,  $\boldsymbol{a_1.b_1} = \sin \alpha \sin \beta \cos \phi$  B1 M1
and hence,
...
*A1 (8)
(iii)  $\boldsymbol{m_1} = \boldsymbol{m} - (\boldsymbol{m.c})\boldsymbol{c} = \frac{1}{2}(\boldsymbol{a} + \boldsymbol{b}) - \left(\frac{1}{2}(\boldsymbol{a} + \boldsymbol{b}).\boldsymbol{c}\right)\boldsymbol{c} = \frac{1}{2}(\boldsymbol{a_1} + \boldsymbol{b_1})$  B1
...
The least popular purely numerical questions (only about 35% answered), the average score is less than 7/20, ranking seventh. The examiner's report pointed out that: (1) The core of the question is to prove properties based on the definition of projection alone, and many candidates directly assume projection properties (such as orthogonality) without proof; (2) The implicit assumptions in geometric diagrams require algebraic verification; (3) Candidates who follow the structure of the question and do dot product calculations carefully usually perform better.
...
Slightly more candidates answered Q2, but the average score was similar to Q4. The examiner's report pointed out that: (1) Almost all candidates can obtain the first intersection condition, and most can obtain a=1/2 from the discriminant; (2) Many candidates have difficulty finding the other two a values ​​(1 and 5), and should consider making dr/dtheta equal; (3) Common problems in drawing: the second curve is drawn as an ellipse, and the cusp is drawn as an angle instead of a smooth depression; (4) Although symmetry is perceived by most people, it is rarely used as a basis for argumentation.
...
The seventh most popular (about 70% responded), with an average score of nearly 8/20, ranking fourth. The examiner's report pointed out that: (1) The derivation is mostly correct; (2) Common errors in drawing: ignoring the discontinuity point at x=tan alpha resulting in two curves, forgetting that the value range is (-pi/2, pi/2), and not marking the intercept/discontinuity point/asymptote/value range; (3) The main problem is the error in handling the negative sign when deriving the derivation in (ii), resulting in only the middle segment being drawn and the constant segments on both sides being omitted.
...
Least successful in purely numerical questions (average score below 6/20). The examiner's report pointed out that: (1) (i) Generally good performance, but missing the absolute value of the module expression and leaking the second possible value of the argument are common deductions; (2) (ii) When dividing the result of (i) by a non-zero quantity, no explanation is given, the vector method and the complex method are confused, and the slope method does not handle the horizontal/vertical special case; (3) (iii) Most people can apply (ii) correctly, but miss the case of h=a when b+c=0; (4) (iv) Many candidates did not answer the question. A common mistake was to describe the transformation as a reflection about a point (which is not a single transformation). The midpoint calculation was also often wrong.
...
The fifth most popular (77% answered), with an average score of about 6.5/20. The examiner's report pointed out that: (1) The induction method is generally performed well, but marks are often lost on logical details; (2) The square inequality in (i) requires attention to the non-negativity of the lower bound, which almost no one notices; there are also difficulties in the basic steps (some people mistakenly believe that 4^0=0); (3) In (ii), when it is proved that |a| value, but did not verify that a > 2 and that the steps are reversible (since division by zero is possible); convergence judgments are mostly informal but correct.
