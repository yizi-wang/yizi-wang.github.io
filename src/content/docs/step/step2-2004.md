...
|---|-------|------------|----------------|
| 1 | Algebra/Equation Solving | Routine | Square root removal, factorization, root testing |
| 2 | Algebra/Inequalities | Standard | Formula, absolute value processing, Vedic theorem |
| 3 | Calculus/Curve Analysis | Standard | Product derivation rule, second-order derivative discrimination method, curve drawing |
| 4 | Calculus/Optimisation | Challenging | Trigonometric function derivation, constrained optimization, elimination method |
| 5 | Integration/Functional Equations | Standard | Integral by parts, expansion of trigonometric identities, method of undetermined coefficients |
| 6 | Vectors | Challenging | Vector projection, dot product operation, orthogonal decomposition |
| 7 | Numerical Methods | Standard | Bisection method, function sign judgment, trigonometric function approximate calculation |
| 8 | Differential Equations | Challenging | Separation of variables method, trigonometric substitution, inequality proof, image analysis |

---

## Question 1

**Topic**: Algebra/Equation Solving Algebra/Equation Solving &nbsp;|&nbsp; **Difficulty**: Routine &nbsp;|&nbsp; **Marks**: 20
...
**Topic**: Algebra/Inequalities Algebra/Inequalities &nbsp;|&nbsp; **Difficulty**: Standard &nbsp;|&nbsp; **Marks**: 20
...
**Topic**: Calculus/Curve Analysis &nbsp;|&nbsp; **Difficulty**: Standard &nbsp;|&nbsp; **Marks**: 20
...
**Topic**: Calculus/Optimisation &nbsp;|&nbsp; **Difficulty**: Challenging &nbsp;|&nbsp; **Marks**: 20
...
**Topic**: Integration/Functional Equations &nbsp;|&nbsp; **Difficulty**: Standard &nbsp;|&nbsp; **Marks**: 20
...
**Topic**: Vectors Vectors &nbsp;|&nbsp; **Difficulty**: Challenging &nbsp;|&nbsp; **Marks**: 20
...
* Again from the data,  $(\mathbf{c}\cdot\mathbf{a})\mathbf{a} = -2\mathbf{a}$  and

 $$|\mathbf{q}|^2 = \mathbf{b}\cdot\mathbf{b} - 6\mathbf{a}\cdot\mathbf{b} + 9\mathbf{a}\cdot\mathbf{a} = 25 - 18 + 9 = 16,$$ 

so  $|\mathbf{q}| = 4$ . Also,

 $$\left[\frac{\mathbf{c}\cdot\mathbf{q}}{|\mathbf{q}|^2}\right]\mathbf{q} = \frac12\mathbf{b} - \frac32\mathbf{a}.$$ 

Thus  $\mathbf{P} = 2\mathbf{a}$ ,  $\mathbf{Q} = -\frac92\mathbf{a} + \frac32\mathbf{b}$ , and  $\mathbf{R} = \frac72\mathbf{a} - \frac12\mathbf{b} + \mathbf{c}$ .
...
**Topic**: Numerical Methods &nbsp;|&nbsp; **Difficulty**: Standard &nbsp;|&nbsp; **Marks**: 20
...
*  $f(3\pi/4) = \sqrt{2} - 3\pi/4$  has the same sign as  $2 - 9\pi^2/16 \approx 2 - 45/8 = -29/8 < 0$ . Hence, as  $f(\pi/2) = 2 - \pi/2 > 0$  and  $f(\pi) = -\pi < 0$ , we have  $I_1 = [\pi/2, 3\pi/4]$ .

* If  $x = \sin(5\pi/8)$ , then  $2x\sqrt{1 - x^2} = 1/\sqrt{2}$ , so  $8x^4 - 8x^2 + 1 = 0$  and hence  $x^2 = \frac12 + \frac{1}{2\sqrt{2}} \approx 0.85$ . The sign of  $f(5\pi/8)$  is the same as that of  $4x^2 - 25\pi^2/64 \approx 17/5 - 125/32 < 0$ . Hence  $I_2 = [\pi/2, 5\pi/8]$ .
