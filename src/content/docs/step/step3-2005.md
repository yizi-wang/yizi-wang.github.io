...
|---|-------|------------|----------------|
| 1 | Trigonometry | Standard | Trigonometric identity transformation, auxiliary angle formula, inequality estimation, function image analysis |
| 2 | Differential Equations and Curves | Challenging | Separation of variables method, implicit function derivation, second-order derivative discrimination method, parameter classification discussion |
| 3 | Polynomials and Equations | Standard | Polynomial compound, undetermined coefficient method, combination method, factorization of quartic equations |
| 4 | Sequences and Recurrence Relations | Challenging | Mathematical induction, simplification of recursion relations, geometric sequence conditions, periodic analysis |
| 5 | Quadratic Curves and Tangents | Challenging | Derivation of tangent equations, discriminant analysis, demonstration of necessary and sufficient conditions, existence of solutions to linear equations |
| 6 | Cubic Equations and Hyperbolic Functions | Hard | Hyperbolic cosine identity, substitution method, cubic equation root formula, complex unit roots |
| 7 | Integration Techniques | Challenging | Variable substitution, partial fractions, radical substitution |
| 8 | Complex Numbers and Geometry | Challenging | Properties of complex modules, conjugate operations, complex equations of circles |

---

## Question 1

**Topic**: Trigonometry &nbsp;|&nbsp; **Difficulty**: Standard &nbsp;|&nbsp; **Marks**: 20
...
**Topic**: Differential Equations and Curves **Difficulty**: Challenging **Marks**: 20
...
**Topic**: Polynomials and Equations &nbsp;|&nbsp; **Difficulty**: Standard &nbsp;|&nbsp; **Marks**: 20
...
Using this in the third equation gives the consistency condition directly:

 $$c=r(2s+p)=\frac{a}{2}\left(b-\frac{a^2}{4}\right).$$ 
...
**Topic**: Sequences and Recurrence Relations &nbsp;|&nbsp; **Difficulty**: Challenging &nbsp;|&nbsp; **Marks**: 20
...
**Topic**: Quadratic Curves and Tangents &nbsp;|&nbsp; **Difficulty**: Challenging &nbsp;|&nbsp; **Marks**: 20
...
**Topic**: Cubic Equations and Hyperbolic Functions | Cubic Equations and Hyperbolic Functions &nbsp;|&nbsp; **Difficulty**: Hard &nbsp;|&nbsp; **Marks**: 20
...
Direct substitution of  $x=2a\cosh(T/3)$  into the left hand side gives

 $$
(2a\cosh(T/3))^3-6a^3\cosh(T/3)
=2a^3\left(4\cosh^3(T/3)-3\cosh(T/3)\right)
=2a^3\cosh T.
$$ 

Let  $a^2=b$  and  $\cosh T=c/a^3$  in the hyperbolic reduction, choosing the sign of  $a$  to match the sign of  $c$  when needed. Then

 $$
T=\ln\left(\frac{c}{a^3}+\sqrt{\frac{c^2}{a^6}-1}\right)
=\ln\left(\frac{c+\sqrt{c^2-b^3}}{a^3}\right)
=3\ln\frac{u}{a}.
$$ 
...
Hence one root is
 ...
 $$
2a\cosh\left(\ln\frac{u}{a}\right)
=2a\cdot\frac{u/a+a/u}{2}
=u+\frac{b}{u}.
$$ 

Since  $u+\frac{b}{u}$  is a root of  $x^3-3bx=2c$ ,

 $$
2c=\left(u+\frac{b}{u}\right)^3-3b\left(u+\frac{b}{u}\right)
=\left(u+\frac{b}{u}\right)\left(u^2+\frac{b^2}{u^2}-b\right).
$$ 

Also

 $$
u^2+\frac{b^2}{u^2}-b-\left(u+\frac{b}{u}\right)^2=-3b,
$$ 
...
so
 ...
 $$
x^3-3bx-2c=
\left(x-\left(u+\frac{b}{u}\right)\right)
\left(x^2+\left(u+\frac{b}{u}\right)x+u^2+\frac{b^2}{u^2}-b\right).
$$ 

The other two roots are therefore the roots of this quadratic:

 $$
\frac{1}{2}\left(-\left(u+\frac{b}{u}\right)\pm
\sqrt{-3\left(u-\frac{b}{u}\right)^2}\right)
=-\frac{1}{2}\left(u+\frac{b}{u}\right)\pm
\frac{i\sqrt{3}}{2}\left(u-\frac{b}{u}\right),
$$ 

that is

 $$\omega u+\omega^2\frac{b}{u}, \qquad \omega^2u+\omega\frac{b}{u}.$$ 

In  $x^3-6x=6$ ,  $b=2$  and  $c=3$ , so  $u=\sqrt[3]{3+1}=2^{2/3}$  and  $b/u=2^{1/3}$ . The solutions are

 $$2^{2/3}+2^{1/3}, \qquad \omega2^{2/3}+\omega^2 2^{1/3}, \qquad \omega^2 2^{2/3}+\omega 2^{1/3}.$$ 
...
For the hyperbolic reduction, set  $a^2=b$  in the case  $b>0$  and choose the sign of  $a$  to match the sign of  $c$ . Then put  $a^3\cosh T=c$ , i.e.  $\cosh T=\frac{c}{a^3}$ .

This requires  $\frac{c}{a^3}\geqslant1$ , which is possible when  $c^2\geqslant b^3$  by the choice of sign for  $a$ .
...
With  $\omega = \frac{1}{2}(-1 + i\sqrt{3})$  and  $\omega^2 = \frac{1}{2}(-1 - i\sqrt{3})$ , we can write the two signs directly as:
...
**Topic**: Integration Techniques | **Difficulty**: Challenging **Marks**: 20
...
Substituting  $u=x^m$  gives

 $$
\int \frac{m}{xf(x^m)}\,dx
=\int \frac{mx^{m-1}}{x^mf(x^m)}\,dx
=\int \frac{du}{uf(u)}
=F(u)+c=F(x^m)+c.
$$ 

For (i), write  $x^n-x=x(x^{n-1}-1)$ . Let  $u=x^{n-1}$  and  $f(u)=u-1$ . Then

 $$
\int \frac{n-1}{x^n-x}\,dx
=\int\frac{du}{u(u-1)}
...
Thus
 ...
 $$
\int\frac{dx}{x^n-x}
=\frac{1}{n-1}\ln\left|\frac{x^{n-1}-1}{x^{n-1}}\right|+c.
$$ 

For (ii), with  $x>0$  and  $n\ne2$ ,

 $$
\int\frac{dx}{\sqrt{x^n+x^2}}
=\int\frac{dx}{x\sqrt{x^{n-2}+1}}.
$$ 

Let  $u=x^{n-2}$  and  $f(u)=\sqrt{u+1}$ . Then

 $$
\int\frac{n-2}{\sqrt{x^n+x^2}}\,dx
=\int\frac{du}{u\sqrt{u+1}}.
$$ 

Now put  $u=v^2-1$ , with  $v>0$ :

 $$
\int\frac{du}{u\sqrt{u+1}}
=\int\frac{2v\,dv}{(v^2-1)v}
=\int\left(\frac{1}{v-1}-\frac{1}{v+1}\right)\,dv
=\ln\left|\frac{v-1}{v+1}\right|.
$$ 
...
Hence
 ...
 $$
\int\frac{dx}{\sqrt{x^n+x^2}}
=\frac{1}{n-2}\ln\left|\frac{\sqrt{x^{n-2}+1}-1}{\sqrt{x^{n-2}+1}+1}\right|+c.
$$ 
...
**Topic**: Complex Numbers and Geometry | Complex Numbers and Geometry **Difficulty**: Challenging **Marks**: 20
...
Use  $|z|^2=zz^*$ . Then
 ...
 $OAC$  is a right angle if and only if  $|AC|^2+|OA|^2=|OC|^2$ ; that is,  $|a-c|^2+|a|^2=|c|^2$ , or using the result above,

 $$2aa^*-ac^*-ca^*=0.$$ 

The circle has centre  $C$  and radius  $AC$ , so points  $z$  on the circle satisfy
...
Because  $OA$  is tangent to the circle, angle  $OAC$  is a right angle and  $2aa^*-ac^*-ca^*=0$ ; thus the condition for points to lie on the circle becomes
...
but multiplying this by  $bb^*\ne0$  gives the same condition.

Conversely, if the points lie on the circle represented by  $|z-c|^2=|a-c|^2$ , compare the two resulting equations:

 $$aba^*b^*-abc^*-ca^*b^*+aa^*=2aa^*-ac^*-ca^*,$$ 

and

 $$\frac{aa^*}{bb^*}-\frac{ac^*}{b^*}-\frac{ca^*}{b}=2aa^*-ac^*-ca^*.$$ 

Multiplying the second equation by  $bb^*$  and using  $bb^*\ne1$  forces  $2aa^*-ac^*-ca^*=0$ , so  $OAC$  is right angled at  $A$  and  $OA$  is tangent to the circle.
