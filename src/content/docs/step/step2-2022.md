...
|---|-------|------------|----------------|
| 1 | Calculus (Calculus) | Standard | Integral by parts, function image analysis, substitution skills, comparison of definite integrals, proof by contradiction |
| 2 | Sequences & Series (Sequences & Series) | Challenging | Recursion relations, algebraic identity transformations, undetermined coefficient method, polynomial functions |
| 3 | Sequences & Series | Challenging | Mathematical induction, summation of series, limit estimation, Fibonacci identity, decimal expansion |
| 4 | Algebra & Functions | Standard | Absolute value segmentation discussion, segmentation function conversion, image drawing, algebra verification |
| 5 | Geometry & Vectors | Hard | Area method, Cauchy-Schwarz inequality, algebraic identities, constrained optimization, inequality proof |
| 6 | Calculus (Calculus) | Challenging | Implicit function differentiation, differential equation solution, orthogonal trajectories, curve family analysis, slope relationship |
| 7 | Complex Numbers | Hard | Calculation of complex module, proof by contradiction, polynomial factorization, Viete's theorem, conjugate root theorem |
| 8 | Matrices & Linear Algebra (Matrices & Linear Algebra) | Challenging | Invariant straight lines, eigenvectors, the relationship between roots of quadratic equations, angle conditions, matrix construction |
...
No official comment. Points prone to errors: (i) The selection of partial integrals u and dv will become more and more complicated if the wrong direction is chosen; (iii) (c) It is necessary to combine the image properties of (a) and the integral equation of (b) to prove that integers m and n do not exist, and students are prone to miss the full use of integer conditions.
...
No official comment. Error-prone points: (i) Deriving the arithmetic from u_{n+1}=(u_{n+2}+u_n)/2 requires correct transfer of terms; (ii) After substituting v_n=t_n+pn^2, it is necessary to verify that t_n satisfies homogeneous recursion; (iii) The substitution of the cubic case requires more careful algebraic operations.
...
No official comment. Error-prone points: (ii) Index offsets are prone to errors in the derivation of series identities, and the upper and lower limits of the sum need to be carefully processed; (iii) Reasoning from infinite series to decimal expansion requires strict error estimation; (iv) Reverse thinking is required to construct rational numbers from Fibonacci decimals.
...
**Topic**: Algebra & Functions &nbsp;|&nbsp; **Difficulty**: Standard &nbsp;|&nbsp; **Marks**: 20
...
No official comment. Error-prone points: (ii) When inferring the absolute value expression from the piecewise function, you need to pay attention to the corresponding relationship between the segmentation points and coefficients; (iii) The processing of |
...
**Topic**: Geometry & Vectors &nbsp;|&nbsp; **Difficulty**: Hard &nbsp;|&nbsp; **Marks**: 20
...
No official comment. Error-prone points: (ii) (b) When finding the minimum distance sum, you need to use the area constraint correctly; (iii) (a) the identity expansion has many terms, and it is easy to miss cross-terms; (iv) When finding the maximum value, you need to consider the situation of P on the boundary, and the processing of constraint conditions is prone to errors.
...
No official comment. Error-prone points: (i) (a) After differentiation of the implicit function, parameter a must be correctly eliminated to obtain the differential equation; (i) (b) At the intersection point on y=x, the slopes of the two tangent lines are the same (both 1), which are not vertical and require special explanation; (ii) When solving the orthogonal trajectory differential equation, attention must be paid to the correctness of the separated variables.
...
No official comment. Error-prone points: (i)(a) Converting the expression h(w)=0 to |w^5| requires clever algebraic operations; (i)(b) When using f(w)-g(w) to disprove, you need to pay attention to the condition of n>=2; (ii)(a) Factorization needs to use |w|=1 and the properties of conjugate; (ii)(d) Judging the number of negative real-part roots requires combining the positive and negative analysis of a_i.
...
No official comment. Error-prone points: (i) bm^2+(a-d)m-c=0 needs to be correctly derived, and the y-axis is treated as a special case; (ii) the sign must be paid attention to when using the angle formula; (iii) the condition for symmetry about y=x is m1*m2=1, and the necessary and sufficient conditions can be obtained by combining the relationship between roots and coefficients.
